 ...
 * Updater for Bukkit.
 *
 * This class provides the means to safely and easily update a plugin, or check to see if it is updated using dev
 * .bukkit.org
 */
...
public class Updater {
    ...
    private static final String HOST = "https://api.curseforge.com"; // Slugs will be appended to this to get to the
    // project's RSS feed

    private static final String[] NO_UPDATE_TAG = {"-DEV", "-PRE", "-SNAPSHOT"}; // If the version number contains
    // one of these, don't update.
    private static final int BYTE_SIZE = 1024; // Used for downloading files
    private YamlConfiguration config; // Config file
    private String updateFolder;// The folder that downloads will be placed in
    private Updater.UpdateResult result = Updater.UpdateResult.SUCCESS; // Used for determining the outcome of the
    // update process
    ...
    public enum UpdateResult {
        ...
        FAIL_DBO,
        /** When running the version check, the file on DBO did not contain the a version in the format 'vVersion'
         * such as 'v1.0'. */
        FAIL_NOVERSION,
        ...
        FAIL_APIKEY,
        /** The updater found an update, but because of the UpdateType being set to NO_DOWNLOAD,
    ...
    public Updater(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
        ...
        if (!updaterConfigFile.exists()) {
            try {
                ...
                plugin.getLogger().severe("The updater could not create a configuration in " + updaterFile
                        .getAbsolutePath());
                e.printStackTrace();
            }
        }
        ...
        this.config.options().header("This configuration file affects all plugins using the Updater system (version " +
                "2+ - http://forums.bukkit.org/threads/96681/ )" + '\n' + "If you wish to use your API key, " +
                "read http://wiki.bukkit.org/ServerMods_API and place it below." + '\n' + "Some updating systems will" +
                " not adhere to the disabled value, but these may be turned off in their plugin's configuration.");
        ...
        if (this.config.get("api-key", null) == null) {
            ...
            try {
                ...
                plugin.getLogger().severe("The updater could not save the configuration in " + updaterFile
                        .getAbsolutePath());
                e.printStackTrace();
            }
        }
    ...
    /** Check to see if the program should continue by evaluation whether the plugin is already updated,
     * or shouldn't be updated */
    private boolean versionCheck(String title) {
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(" v").length == 2) {
                ...
                final String authorInfo = this.plugin.getDescription().getAuthors().size() == 0 ? "" : " (" + this
    ...
    private boolean read() {
        try {
            ...
            if (e.getMessage().contains("HTTP response code: 403")) {
                this.plugin.getLogger().warning("dev.bukkit.org rejected the API key provided in " +
                ...
                this.plugin.getLogger().warning("If you have not recently modified your configuration and this is the" +
